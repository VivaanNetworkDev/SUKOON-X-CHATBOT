...
from SUKOONXCHATBOT.database import sukoon
...
async def cb_handler(_, query: CallbackQuery):
    if query.data == "HELP":
        ...
        if user_status not in [CMS.OWNER, CMS.ADMINISTRATOR]:
            ...
            is_sukoon = sukoon.find_one({"chat_id": query.message.chat.id})
            if not is_sukoon:
                await query.edit_message_text(f"**ᴄʜᴀᴛ-ʙᴏᴛ ᴀʟʀᴇᴀᴅʏ ᴇɴᴀʙʟᴇᴅ.**")
            if is_sukoon:
                sukoon.delete_one({"chat_id": query.message.chat.id})
        ...
        if user_status not in [CMS.OWNER, CMS.ADMINISTRATOR]:
            ...
            is_sukoon = sukoon.find_one({"chat_id": query.message.chat.id})
            if not is_sukoon:
                sukoon.insert_one({"chat_id": query.message.chat.id})
                await query.edit_message_text(
